...
| `eventos` | **Pública solo si está aprobada.** El dueño y el administrador ven las pendientes. Una consulta que alcance una pendiente **falla entera**, no devuelve menos filas. | Nace pendiente y con la fecha del servidor —`fechaCreacion == request.time`, el único valor de tiempo que un cliente no puede inventar—. El identificador del documento tiene que coincidir con `idEvento`, solo se admiten los catorce campos del modelo, y `tituloNormalizado` no puede decir algo distinto del título ([20 §4 y §5](20-publicacion-de-eventos.md)). Al editarla vuelve a pendiente (HU-23). Editar exige escribir `pendiente` y no tocar `idEvento`, `idActor`, `fechaCreacion` ni `contadorConsultas`: de ahí sale solo el «vuelve a revisión» de HU-23. **Eliminar no mira el estado** —retirar lo propio del catálogo no pide permiso; cambiar lo aprobado sí ([22 §5](22-edicion-y-eliminacion.md))—. Las dos reglas estaban escritas desde HU-21 y sin un solo caso de prueba hasta HU-22 y HU-23 ([22 §1](22-edicion-y-eliminacion.md)). HU-25 no cambió nada aquí: lo que añadió son casos que preguntan si **pedir menos documentos equivale a pedir los permitidos** —con `limit`, con orden y con cursor—, y las tres veces la respuesta es que no ([24 §2](24-catalogo-publico.md)). HU-26 lo repitió con los filtros: un `where` sobre la categoría se parece muchísimo al del estado y tampoco hace su trabajo. |
| `categorias` | Pública: los filtros la necesitan sin sesión. | Solo el administrador (RF-14). |
| `moderaciones` | El administrador, y el actor dueño del evento moderado. | Solo el administrador. **Devolver exige observación escrita** (HU-24), y la observación se recorta antes de medirla: tres espacios tenían tamaño tres y pasaban. Desde HU-24 la regla exige además la forma que el modelo ya declaraba —`idModeracion` igual al identificador del documento, `idEvento`, observaciones nulas al aprobar y ningún campo de más—. Registro inmutable: no se edita ni se borra, y hay dos casos que lo comprueban ([23 §4](23-moderacion.md)). |
| `interacciones` | Solo el administrador: alimentan los indicadores (HU-34). Ni siquiera el actor dueño del evento lee las suyas, y es deliberado: cruzar la hora del registro con la del mensaje que recibió reconstruiría el dato que RNF-06 dice no recoger ([28 §7](28-contacto-directo.md)). | Cualquiera puede añadir una, pero **solo con los cuatro campos declarados y todos ellos**: hasta HU-29 `hasOnly` admitía documentos **sin** `idEvento`, que ningún indicador puede contar ([28 §5](28-contacto-directo.md)). Nunca datos identificables del visitante (RNF-06). El registro es inmutable, y desde HU-29 hay dos casos que lo comprueban. |
...
HU-25 añadió un séptimo, `estadoPublicacion` + `fechaFin`, y HU-26 tres más —uno por
cada combinación de filtros ([25 §9](25-filtros-del-catalogo.md))—. Los cuatro se
publican aparte y **antes** de fusionar ([24 §5](24-catalogo-publico.md)):
